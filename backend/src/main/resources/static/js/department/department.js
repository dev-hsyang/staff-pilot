...
function showButtonDepHierarchy(superId, depMap, minLevel) {
    ...
    for (let dep of depMap.get(superId)) {
        ...
        $('#dep-list').append(
            '<div>' +
            '<a href="https://api.staff-pilot.net/corporation'+ getNextPath(window.location.href, PATH_COR) +'/dep/' + dep.id + '">' + (spaces.repeat(dep.level-minLevel)) + (dep.level-minLevel > 0 ? 'ㄴ' : '') + dep.name + ' </a>' +
            '</div>'
        );
